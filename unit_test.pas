...
function TForm3.resourceVersionInfo: string;
...
procedure TForm3.ShowVersionInfo;
begin
...
function TForm3.JNochZuMelden(Sender: TObject): double;
var row : integer;
...
function TForm3.IsRIDinSelection(RID: string): boolean;
var
  row: integer;
begin
  try
...
function TForm3.ShowFormSum(var Titel, Betrag: TStringList): boolean;
var
  x: integer;
begin
  try
...
function TForm3.CheckFilterPossible(FieldName: String): Boolean;
begin
...
function TForm3.LookUpStringGrid(Grid: TStringGrid; GesuchterMandant, ZuErsetzendeLinienNr: string): string;
...
procedure TForm3.LinieJeMandant(Sender: TObject);
...
function TForm3.AddFilterHistory(item: String): boolean;
begin
  Result := false;
...
procedure TForm3.ShowRecordCount(Sender: TObject);
begin
...
function TForm3.ExistInDb(TableName: string): boolean;
var
  TableFound: boolean;
...
function TForm3.ExistField(FieldName: string): boolean;
var
  x: integer;
begin
  try
...
function TForm3.MarkExported(DBFName: string; mark: string; IDs: TStringList): boolean;
var
  tab: TDBF;
  Betrag: currency;
  v: variant;
...
function TForm3.IsDate(str: string): boolean;
var
  dt: TDateTime;
begin
  Result := True;
...
function TForm3.ShowFilterInfo(Warning: boolean): boolean;
begin
...
procedure TForm3.BereitsGemeldetWurden(Sender: TObject);
var
  MyDBF: TDBF;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if DirectoryExists(Form3.DirectoryEdit1.Text) then
            begin
              FName := IncludeTrailingBackslash(Form3.DirectoryEdit1.Text) +
                'RIDGeloescht.txt';
            end
            else
            begin
...
procedure TForm3.CheckIniFile(Sender: TObject);
var
  ini1, ini2: TIniFile;
...
begin
  ...
  Section := 'TApplication.Form3.Edit_last_buchung_Items';
...
procedure TForm3.angezeigteDatenkopieren1Click(Sender: TObject);
var
  {* Siehe auch:
...
begin
  try
    ...
    Form3.mem.Text := mem1.Text;
    Form3.mem.SelectAll;
    Form3.mem.CopyToClipboard;
    Form3.mem.Clear;
    Form3.mem.Visible := False;
...
function TForm3.OpenLog(FName: string): boolean;
begin
  try
...
procedure TForm3.ShortenLog(anzLines: integer; Sender: TObject);
var
  log: TStringList;
  x: integer;
begin
...
procedure TForm3.IniPropStorage1RestoreProperties(Sender: TObject);
begin
  TarifVersion := SpinEditTarifversion.Value;

end;

procedure TForm3.IniPropStorage1StoredValues1Restore(Sender: TStoredValue;
  var Value: TStoredType);
begin
  SavePath := Value;
  (* Fehler verhindern *)
...
procedure TForm3.IniPropStorage1StoredValues1Save(Sender: TStoredValue;
  var Value: TStoredType);
begin
...
procedure TForm3.IniPropStorage1StoredValues2Restore(Sender: TStoredValue;
  var Value: TStoredType);
begin
  if Value = '' then Value := FormatDateTime('dd.mm.yyyy', incMonth(Date,-1));
end;

procedure TForm3.kopierenClick(Sender: TObject);
begin
  angezeigteDatenkopieren1Click(Sender);
end;

procedure TForm3.ListBoxKnownLinesKeyDown(Sender: TObject; var Key: word;
  Shift: TShiftState);
begin
...
procedure TForm3.ListFieldsClick(Sender: TObject);
var
  x: integer;
begin
  try
...
procedure TForm3.LookupStornosClick(Sender: TObject);
var
  Filter: string;
  x, y: integer;
  Betrag: currency;
...
procedure TForm3.MenuAddLinieClick(Sender: TObject);
var
  UserString: string;
begin
...
procedure TForm3.MnAnrufsammeltaxisClick(Sender: TObject);
begin
...
procedure TForm3.MNCheckLinieJeMandantClick(Sender: TObject);
begin
  LinieJeMandant(Sender);
end;

procedure TForm3.MnFieldlistClick(Sender: TObject);
begin
...
procedure TForm3.MnFixColumnClick(Sender: TObject);
begin
...
procedure TForm3.MnGroupValuesClick(Sender: TObject);
...
procedure TForm3.MnManuelleBuchungenClick(Sender: TObject);
begin
...
procedure TForm3.MnMarkExportedClick(Sender: TObject);
var
  FName, mark: string;
  MyDirSelect: TSelectDirectoryDialog;
begin
...
procedure TForm3.MnSearchClick(Sender: TObject);
var Col, x, y : Integer;
    found : boolean;
begin
...
procedure TForm3.MnSetAST_TarifVersionClick(Sender: TObject);
var OldSQL : string;
begin
  try
...
procedure TForm3.MnShowRecordCountClick(Sender: TObject);
begin
   ShowMessage('Angezeigt werden ' + FormatFloat('#,##0',QFaks.RecordCount) + ' Datensätze.');
end;

procedure TForm3.MnSucheLinienClick(Sender: TObject);
begin
...
procedure TForm3.MnSumColumnClick(Sender: TObject);
var
  x: integer;
  s: Float;
  JFieldName: string;
...
procedure TForm3.OpenLogFileClick(Sender: TObject);
begin
  OpenLog(ZSQLMonitor1.FileName);
end;

procedure TForm3.Pm_SearchClick(Sender: TObject);
var
  gefunden: boolean;
begin
...
procedure TForm3.PopupGridClose(Sender: TObject);
begin
      Application.ProcessMessages;
end;

procedure TForm3.PopupGridPopup(Sender: TObject);
begin
...
procedure TForm3.RemoveFilterClick(Sender: TObject);
begin
  QFaks.Filtered := False;
  QFaks.Filter := '';
  FilterCombo.Text := QFaks.Filter;
...
procedure TForm3.RID_as_FilterClick(Sender: TObject);
var
  Filter, StrOr: string;
  x, MaxRecs: integer;
  tab : TDBF;
...
procedure TForm3.SaveToFileClick(Sender: TObject);
begin
...
procedure TForm3.SpinEditTarifversionEditingDone(Sender: TObject);
begin
  Memo1.SetFocus;
  ShowMessage('Bitte neu starten!');
end;

procedure TForm3.SQLLoadClick(Sender: TObject);
begin
...
procedure TForm3.UniqueInstance1OtherInstance(Sender: TObject;
  ParamCount: integer; Parameters: array of string);
begin
...
procedure TForm3.QFaksAfterOpen(DataSet: TDataSet);
var
  x: integer;
begin
...
procedure TForm3.ApplicationProperties1Hint(Sender: TObject);
begin
  StatusBar1.SimpleText := Application.Hint;
end;

procedure TForm3.AuswahlFilterClick(Sender: TObject);
var
  Filter: string;
  row: integer;
begin
...
procedure TForm3.BtnAddLineClick(Sender: TObject);
begin
  (* Zeile hinzufügen *)
...
procedure TForm3.BtnDelLineClick(Sender: TObject);
begin
  GridReplaceLineNumber.SetFocus;
...
procedure TForm3.ApplicationProperties1Exception(Sender: TObject; E: Exception);
begin
...
procedure TForm3.CbincMonthClick(Sender: TObject);
begin
...
procedure TForm3.CheckLinieClick(Sender: TObject);
var
  x, y: integer;
  lst: TStringList;
begin
...
procedure TForm3.DateEditVonAcceptDate(Sender: TObject; var ADate: TDateTime;
  var AcceptDate: boolean);
begin
  try
    AcceptDate := True;
...
procedure TForm3.DBase_exportClick(Sender: TObject);
var
...
procedure TForm3.DBGridFaksDblClick(Sender: TObject);
begin
...
procedure TForm3.DBGridFaksPrepareCanvas(sender: TObject; DataCol: Integer;
  Column: TColumn; AState: TGridDrawState);
begin
...
procedure TForm3.DBGridFaksTitleClick(Column: TColumn);
const
...
procedure TForm3.DeleteSelectedClick(Sender: TObject);
var
  x: integer;
begin
...
procedure TForm3.DirectoryEdit1ButtonClick(Sender: TObject);
begin
  DirectoryEdit1.RootDir := ExePath;
end;

procedure TForm3.EventLog_anzeigenClick(Sender: TObject);
begin
  (* Eventlog anzeigen *)
...
procedure TForm3.ExportExcelClick(Sender: TObject);
begin
  try
    jei;
...
procedure TForm3.FilterAbClick(Sender: TObject);
var
  Filter: string;
begin
  (* Filter auf Felder nur mit Zeit kann ich nicht *)
...
procedure TForm3.FilterBisClick(Sender: TObject);
var
  Filter: string;
begin
  (* Filter auf Felder nur mit Zeit kann ich nicht *)
...
procedure TForm3.FilterComboKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
...
procedure TForm3.FilterSetzenClick(Sender: TObject);
var
  Filter: string;
begin
  if QFaks.Filtered then
...
procedure TForm3.JEi;
begin
  screen.cursor := crHourglass;
end;

procedure TForm3.NEi;
begin
  screen.cursor := crDefault;
end;

function TForm3.Verbinden: boolean;
var
  d, m, y, d1, m1, y1: word;
...
procedure TForm3.BtnExportExcelClick(Sender: TObject);
begin
  DBGridFaks.SetFocus;
...
procedure TForm3.SumClick(Sender: TObject);
var
...
procedure TForm3.QFaksBeforeOpen(DataSet: TDataSet);
begin
...
(* wichtig: Damit das Ü in Grünberg erhalten bleibt: PWideChar(UTF8Decode(Zeile)) *)
function TForm3.ExportDatasetToExcel(JDataset: TDataset): boolean;
var
...
function TForm3.OpenExplorer(FName: string): boolean;
begin
  try
...
procedure TForm3.QFaks2DbaseClick(Sender: TObject);
var
  x: integer;
  Dbase: TDBF;
begin
...
*)
procedure TForm3.UpdateElgebaLinienNrClick(Sender: TObject);
var Elgeba : TDBF;
    x : integer;
begin
  try
